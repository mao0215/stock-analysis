...
*   **Signal Detection**: Scores stocks (0-6) based on 6 technical criteria:
...
*   **Signal Start Date**: Identifies exactly when a stock started meeting the criteria.
...
3.  **Check Results**
    *   The console will display recent signals and top-ranking stocks.
    *   Full results are saved to `analysis_results.csv`.
...
*   **シグナル検出**: 以下の6つのテクニカル指標に基づき、銘柄をスコアリング（0〜6点）します。
    ...
    5.  上昇時の出来高増・下落時の出来高減
    6.  週足での出来高分析
*   **シグナル点灯日特定**: その銘柄が「いつから」条件を満たし始めたかを特定します。
...
3.  **結果の確認**
    *   コンソールに直近でシグナルが出た銘柄や、高スコアの銘柄が表示されます。
    *   全銘柄の詳細な分析結果は `analysis_results.csv` に保存されます。
